...
{ // step1, voxelization and plane dection
  ...
  // get_project_plane() and merge_plane() allocate temporary merged planes.
  // The reference implementation leaves them behind for every descriptor
  // frame, which is material on long offline bags. They are no longer needed
  // once the binary descriptors have been extracted.
  std::unordered_set<BTCPlane *> temporary_planes;
  temporary_planes.insert(proj_plane_list.begin(), proj_plane_list.end());
  temporary_planes.insert(merge_plane_list.begin(), merge_plane_list.end());
  for (BTCPlane *plane : temporary_planes) {
    delete plane;
  }
