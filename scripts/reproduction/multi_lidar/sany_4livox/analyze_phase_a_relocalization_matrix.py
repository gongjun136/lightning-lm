...
def summarize(
    ...
    summary: dict[str, object] = {
        ...
        "minimum_overlap": min((float(trial["overlap"]) for trial in accepted), default=None),
        "minimum_confirmations": min((int(trial["confirmations"]) for trial in accepted), default=None),
        "unsafe_overlap_accepts": sum(float(trial["overlap"]) < 0.95 for trial in accepted),
...
def main() -> None:
    ...
    parser.add_argument(
        "--reference-runs-root",
        type=Path,
        help="Optional same-offset BTC runs used as trajectory pseudo-truth where accepted",
    )
    ...
    report = {
        ...
        "criteria": {
            ...
            "reference_scope": "only trials accepted by the historical BTC baseline",
