#!/usr/bin/env bash
# 通用在线 SLAM 运行模板，对应 src/app/run_slam_online.cc。
#
# 适用场景：
#   订阅实时 ROS2 话题进行在线建图；用于接传感器、rosbag play 或在线调试。
#
# 常用环境变量：
...
#   其余参数会原样追加给 run_slam_online。
#
# 输出组织：
#   $LIGHTNING_LM_OUT_ROOT/<run_name>/run_metadata.txt
#   $LIGHTNING_LM_OUT_ROOT/<run_name>/logs/run_slam_online.{stdout,stderr}.log
#   程序会在 $LIGHTNING_LM_OUT_ROOT/<run_name>/ 内执行，后续保存地图时相对路径会落在该目录下。
#
# 示例：
#   LIGHTNING_LM_CONFIG=/path/to/config.yaml \
