#!/usr/bin/env bash
# 通用前端 LIO 离线运行模板，对应 src/app/run_frontend_offline.cc。
#
# 适用场景：
#   只运行前端 LIO，不进入后端 SLAM；用于快速验证里程计、ESKF 参数和前端轨迹精度。
#
# 必需环境变量：
...
#       离线处理完是否等待 Pangolin UI 关闭；需要可视化时设为 true。
#   LIGHTNING_LM_MAX_LIDAR_FRAMES=N
#       只跑前 N 帧点云；0 表示完整运行。
...
#   其余参数会原样追加给 run_frontend_offline，例如 --max_lidar_frames=300。
#
# 输出组织：
#   $LIGHTNING_LM_OUT_ROOT/<run_name>/run_metadata.txt
#   $LIGHTNING_LM_OUT_ROOT/<run_name>/logs/run_frontend_offline.{stdout,stderr}.log
...
#   LIGHTNING_LM_WAIT_UI=true \
#   scripts/run_frontend_offline.sh outdoor01_frontend_test
set -euo pipefail
