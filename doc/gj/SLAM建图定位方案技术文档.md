...
详细字段定义见 [消息定义.xlsx](./消息定义.xlsx)。本章只对建图定位链路中使用的输入、输出和服务消息做文字说明。当前系统确定使用 Livox 雷达，但具体点云消息类型取决于上游驱动封装方式：若驱动发布标准 ROS2 点云，则使用 `common.lidar_topic` 对应的 `sensor_msgs/msg/PointCloud2`；若驱动发布 Livox 原生自定义点云，则使用 `common.livox_lidar_topic` 对应的 `livox_ros_driver2/msg/CustomMsg`。两类点云输入在前端预处理后都会被统一转换为内部带逐点时间的点云格式，供去畸变、匹配和建图使用。

### 9.1 IMU 输入

`common.imu_topic` 使用 `sensor_msgs/msg/Imu`，是建图和定位共同依赖的惯性测量输入。系统主要读取 `header.stamp`、`angular_velocity` 和 `linear_acceleration` 字段：时间戳用于与 LiDAR 扫描帧同步，角速度和线加速度用于 IMU 传播、静态初始化、点云运动畸变补偿和定位阶段的高频外推。`orientation` 和各协方差字段当前不作为直接观测使用，滤波噪声和协方差主要由配置参数给出。

IMU 时间戳必须与 LiDAR 使用同一时间基准，并保持单调递增。如果 IMU 时间无法覆盖当前点云帧的完整扫描周期，前端会等待后续 IMU；如果时间回退或单位错误，会直接影响重力初始化、运动预测和去畸变质量。加速度默认按 `m/s^2` 理解，角速度默认按 `rad/s` 理解。

### 9.2 标准点云输入

`common.lidar_topic` 使用 `sensor_msgs/msg/PointCloud2`，用于接入被驱动转换成标准 ROS2 点云格式的 Livox 数据。系统读取消息头时间戳作为点云帧起始时间，读取点云字段描述和二进制数据完成点解析。对 SLAM 前端而言，最关键的是点坐标和扫描内逐点相对时间；点坐标用于构造 scan-to-map 几何残差，逐点时间用于把一帧扫描内不同时刻采样的点补偿到同一参考时刻。

如果 Livox 驱动最终输出 `PointCloud2`，需要确认其 `fields` 中包含前端预处理能够识别的坐标、强度和时间字段。对于当前方案，`PointCloud2` 只是标准容器，具体字段名称和时间单位仍要以驱动实际发布为准；接入时应通过 `ros2 topic echo` 或 bag 元数据核对字段。若逐点时间缺失或单位不一致，系统仍可能接收到点云，但去畸变和匹配精度会明显下降。

### 9.3 Livox 自定义点云输入

`common.livox_lidar_topic` 使用 `livox_ros_driver2/msg/CustomMsg`，用于接入 Livox 驱动原生自定义点云。该消息包含帧级 `header`、`timebase`、点数量 `point_num`、设备编号 `lidar_id` 和 `points` 点数组。每个 `CustomPoint` 包含 `offset_time`、`x/y/z`、`reflectivity`、`tag` 和 `line`。当前前端主要使用 `header.stamp` 作为扫描帧时间，使用 `offset_time` 作为点在帧内的相对采样时间，使用 `x/y/z` 和 `reflectivity` 生成内部点云。

Livox 非重复扫描雷达的点云运动补偿非常依赖 `offset_time`。当前实现会把 `offset_time` 按纳秒到毫秒的尺度转换后写入内部点时间字段，因此驱动侧必须保证 `offset_time` 与 `header.stamp` 属于同一帧时间定义。`tag` 和 `line` 字段在消息中保留，后续可用于更细粒度的点质量过滤或线束分析，但当前方案默认不依赖它们完成主流程。

### 9.4 定位 TF 输出

`/tf` 使用 `geometry_msgs/msg/TransformStamped`，是在线定位阶段对外发布的主要位姿结果。当前输出的父坐标系为 `map`，子坐标系为 `base_link`，表示机体坐标系相对于全局地图坐标系的平移和旋转。平移字段单位为 m，旋转字段为单位四元数。

该 TF 来自定位滑窗 PGO 的高频输出：低频的全局点云匹配结果提供绝对约束，LIO/IMU 短时运动提供连续外推。上层导航、可视化或任务系统如果只需要车辆当前全局位姿，优先订阅该 TF。若需要显式定位状态、置信度或退化原因，则需要在现有状态回调基础上扩展独立定位状态消息。

### 9.5 保存地图服务

`/save_map` 使用 `srv/SaveMap`，用于建图完成后触发地图落盘。请求字段 `map_id` 表示地图名称或保存目录名；响应字段 `response` 表示保存结果。当前方案中，保存流程会导出全局点云、分块点云地图，并在启用 2D 栅格模块时同步保存 `map.pgm` 和 `map.yaml`。

保存地图通常在建图链路已经积累足够关键帧、回环优化稳定后调用。`map_id` 应保持唯一且可追溯，避免覆盖已有地图。响应码中 `0` 表示成功，非零值用于表达保存中、地图为空、渲染失败或扩图定位失败等异常；工程实现中可根据实际保存流程进一步细化错误返回。

### 9.6 定位命令服务

`/loc_cmd` 使用 `srv/LocCmd`，用于定位阶段的地图切换和重定位控制。消息中定义了三类命令：`CMD_LOC_SET_MAP=1` 用于设置或切换定位地图，`CMD_LOC_INIT_POSE=3` 用于设置重定位初始位姿，`CMD_LOC_INIT_LIST=4` 用于设置候选重定位点列表。请求中的 `id` 指明命令类型，`map_id` 指向地图路径或地图标识，`x/y/z` 用于传入外部初始位姿参数，`header` 用于携带命令时间和坐标系信息。

该服务面向上层调度或人工重定位工具。设置地图命令通常需要释放旧地图并重新加载分块地图索引；设置初始位姿命令会给 NDT 全局匹配提供初值，降低搜索范围并提高收敛稳定性。需要注意的是，`z` 字段在当前定义中既可能表示三维高度，也可能在二维重定位场景中承载 yaw 参数，接入上层系统时应统一约定其语义，避免初始化姿态解释不一致。
